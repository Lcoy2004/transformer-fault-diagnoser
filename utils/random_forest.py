...
def train_random_forest(db_path='database/fault_data.db', n_estimators=100, random_state=42, progress_callback=None, progress_value_callback=None):
    """
    ...
    try:
        # 1. 从数据库读取PCA降维后的数据
        send_notification("从数据库读取PCA降维后的数据...")
        ...
        if df.empty:
            error_msg = "fusion_features表为空，请先运行train_pca.py生成降维数据"
            logger.error(error_msg)
            send_notification(error_msg)
            raise ValueError("缺少PCA降维数据")
        
        logger.info(f"成功读取PCA降维数据，形状: {df.shape}")
        send_notification(f"成功读取PCA降维数据，形状: {df.shape}")
...
if __name__ == "__main__":
    ...
    logger.info(f"随机森林模型训练完成")
    logger.info(f"准确率: {result['accuracy']:.4f}")
    logger.info(f"模型已保存到: {result['model_path']}")
