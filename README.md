...
本项目是一款变压器故障诊断软件，采用**油色谱分析（DGA）**和**局部放电超声波检测（AE-PD）**监测技术，结合**PCA降维**和**随机森林分类**算法，实现对变压器故障类型、位置的智能诊断。

### 核心特性

| 特性 | 说明 |
|------|------|
| 多源数据融合 | 支持DGA油色谱数据 + 超声波PD局部放电数据联合诊断 |
| 智能算法 | PCA降维 + 随机森林分类 |
...
1. **数据导入** - 导入DGA油色谱数据、超声波PD局部放电数据（Excel格式）
...
| 仅PD | 仅使用超声波PD局部放电数据判断放电细类 | 只有PD数据 |
